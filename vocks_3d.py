...
def main(train_batch_size=64, epochs=2000, sample_batch_size=64):
    ...
    visualize_house((houses_torch[33].permute(1,2,3,0) + 1) / 2, f'{NAME}/sample1.png')
    visualize_house((houses_torch[80].permute(1,2,3,0) + 1) / 2, f'{NAME}/sample2.png')
    visualize_house((houses_torch[128].permute(1,2,3,0) + 1) / 2, f'{NAME}/sample3.png')
    ...
    for ns in training_loop(loader, model, schedule, epochs=epochs, lr=2e-4, accelerator=a):
        ns.pbar.set_postfix(loss=f'{ns.loss.item():.5}')
        ema.update()
