...
def main(train_batch_size=32, epochs=300, sample_batch_size=64):
    ...
    NAME = '3d_cube' + timestr

    # Setup
    a = Accelerator()

    # 8x8x8 with a 16x16x16 cube of a random color in the center
    DSIZE = 2000
    cubes = torch.zeros(DSIZE, 4, 8, 8, 8)
    for i in range(DSIZE):
        x = torch.randint(0, 5, (3,))
        color = torch.rand(4)
        color[3] = 1
        color = color.unsqueeze(1).unsqueeze(2).unsqueeze(3)
        cubes[i, :, x[0]:x[0]+3, x[1]:x[1]+3, x[2]:x[2]+3] = color
    # b, c, x, y, z
